...
func TestClassifyVersionWitness(t *testing.T) {
	tests := []struct {
		...
		{name: "Homebrew HEAD stamp", version: "HEAD-f925f3f", want: witnessEraCurrent},
		{name: "bare Homebrew HEAD stamp", version: "HEAD", want: witnessEraCurrent},
		{name: "Homebrew HEAD stamp with revision", version: "HEAD-f925f3f_1", want: witnessEraCurrent},
...
// #5603: Homebrew stamps HEAD-<shortsha> into main.Version for --HEAD installs,
// bd writes it verbatim, and the workspace must open without a warning that can
// never self-heal while the same HEAD build remains installed.
func TestLegacyUpgradeGuardAdmitsBrewHeadStamp(t *testing.T) {
	for _, stamp := range []string{"HEAD-f925f3f", "HEAD", "HEAD-f925f3f_1"} {
		t.Run(stamp, func(t *testing.T) {
			...
			if warnings.Len() != 0 {
				t.Fatalf("guard warned about a recognized Homebrew HEAD stamp: %q", warnings.String())
			}
		})
	}
}
